...
exports.createLocation = function(req, res) {
	...
	PhotoLocation.find({dataSet: datasetId}).limit(1).sort({$natural:-1}).exec(function(err, pLocation) {
		if (pLocation)
...
exports.listLocations = function(req, res) {
	PhotoLocation.find({dataSet: datasetId}).populate('dataSet previousLocation').exec(function(err, pLocations) {
		if (err) {
			return res.status(400).send({
				message: getErrorMessage(err)
			});
